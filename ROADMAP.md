...
Only four domains are wired to real data: **Templates, Client Templates, New Matter (create), and the Dashboard's case/status numbers.** Everything else — Workflow (kanban), Signed Contracts, Reporting (revenue/billable-hours charts), Contract Data, Matter Admin (permissions table), Request Support, Resources, Learned Friend (AI assistant), LightHub Guide, Integrations — is static/local mock data with **no backing backend model at all.** These aren't half-built; they're UI mockups for domains the backend hasn't been designed for yet.

### What this means

The core "system of record" (firms → clients → matters → templates, with proper multi-tenant isolation) is solid and genuinely production-shaped. Everything LightHub advertises *beyond* that — contract lifecycle analytics, workflow/permission controls, spend data, capacity planning, a knowledge repository, integrations — currently exists only as frontend set-dressing.
...
- "Learned Friend" AI assistant: wire to the Claude API (Messages API) scoped to a firm's own matters/documents so it can answer questions grounded in real firm data — this is a strong differentiator LightHub doesn't appear to advertise at all
...
- Audit log (who changed what, when) — natural extension of the firm-isolation work already done
