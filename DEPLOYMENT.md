...
Provisioned via `render.yaml` (§5) as `legalhub-redis`, a Render "Key Value" managed instance —
...
(`legalhub-api`), the Celery worker+beat process (`legalhub-worker`), and a managed Redis
(`legalhub-redis`, Render's "Key Value" service) as one unit, reusing `backend/Procfile`'s two
...
3. Once the Blueprint is live, open each of `legalhub-api`/`legalhub-worker`/`legalhub-frontend`'s
...
4. **Only one `legalhub-worker` instance should ever run** — a second would duplicate every
