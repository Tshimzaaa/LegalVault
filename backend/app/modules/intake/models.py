...
class IntakeForm(BaseModel):
    ...
    # True only for the per-org "Contract Request" form seeded at org creation (see
...
class IntakeFormField(BaseModel):
    ...
    # without depending on a human-editable label or an org-specific field UUID. Never
...
class IntakeSubmission(BaseModel):
    """Submitted by any staff member (no external client tier exists in this
    corporate/in-house model) — e.g. someone in procurement requesting a new
    supplier contract, which legal then reviews and converts into a Contract."""
    ...
    converted_contract_id: Mapped[UUID | None] = mapped_column(
