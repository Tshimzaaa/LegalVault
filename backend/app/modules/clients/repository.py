...
class ClientRepository:
    ...
    def create_contact_invite(self, client_id, first_name: str, last_name: str, email: str) -> ClientContact:
        ...
        self.db.flush()
        return contact
    def list_by_org(self, org_id) -> list[Client]:
        return list(self.db.scalars(select(Client).where(Client.org_id == org_id)))
    ...
    def clear_reset_token(self, contact: ClientContact) -> ClientContact:
        ...
        self.db.flush()
        return contact
    def count_clients_for_org(self, org_id) -> int:
        from sqlalchemy import func
        return self.db.scalar(select(func.count()).select_from(Client).where(Client.org_id == org_id))
