...
def test_matter_due_soon_notifies_each_assigned_staff_user(db_session):
    ...
    lawyer, _ = make_staff(db_session, firm)
    paralegal, _ = make_staff(db_session, firm, email="paralegal@example.com")
    client_company = make_client_company(db_session, firm)
    matter = make_matter(db_session, firm, client_company, due_date=TODAY + timedelta(days=3))
...
def test_matter_not_due_soon_excluded(db_session):
    ...
    lawyer, _ = make_staff(db_session, firm)
    client_company = make_client_company(db_session, firm)
    matter = make_matter(db_session, firm, client_company, due_date=TODAY + timedelta(days=30))
    make_matter_assignment(db_session, matter, lawyer)
...
def test_matter_closed_or_declined_excluded(db_session):
    ...
    for status in (MatterStatus.CLOSED, MatterStatus.DECLINED):
        matter = make_matter(db_session, firm, client_company, due_date=TODAY + timedelta(days=1), status=status)
        make_matter_assignment(db_session, matter, lawyer)
...
def test_matter_inactive_assignee_excluded(db_session):
    firm = make_firm(db_session)
    lawyer, _ = make_staff(db_session, firm, is_active=False)
    client_company = make_client_company(db_session, firm)
    matter = make_matter(db_session, firm, client_company, due_date=TODAY + timedelta(days=1))
    make_matter_assignment(db_session, matter, lawyer)

    assert scan_and_notify_due_matters(db_session) == 0


def test_suspended_firm_excluded(db_session):
    firm = make_firm(db_session, is_active=False)
    lawyer, _ = make_staff(db_session, firm)
    client_company = make_client_company(db_session, firm)
    matter = make_matter(db_session, firm, client_company, due_date=TODAY + timedelta(days=1))
    make_matter_assignment(db_session, matter, lawyer)
...
def test_contract_expiring_notifies_uploader(db_session):
    firm = make_firm(db_session)
    uploader, _ = make_staff(db_session, firm)
    client_company = make_client_company(db_session, firm)
    contract = make_signed_contract(
        db_session, firm, client_company, uploaded_by=uploader.id, expiry_date=TODAY + timedelta(days=10)
    )
...
def test_contract_no_expiry_date_excluded(db_session):
    ...
    uploader, _ = make_staff(db_session, firm)
    client_company = make_client_company(db_session, firm)
    make_signed_contract(db_session, firm, client_company, uploaded_by=uploader.id)
...
def test_contract_not_expiring_soon_excluded(db_session):
    ...
    client_company = make_client_company(db_session, firm)
    make_signed_contract(
        db_session, firm, client_company, uploaded_by=uploader.id, expiry_date=TODAY + timedelta(days=90)
    )
...
def test_contract_no_uploader_gets_no_reminder(db_session):
    ...
    admin at the firm) was deliberately ruled out rather than left as an accidental oversight."""
    firm = make_firm(db_session)
    client_company = make_client_company(db_session, firm)
    make_signed_contract(
        db_session,
