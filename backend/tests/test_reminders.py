"""
Contract due-date and contract expiry reminder scans (app/tasks/reminders.py). These call
...
from app.modules.contracts.models import ContractStage
...
from app.tasks.reminders import (
    ...
    CONTRACT_REMINDER_TYPE,
    scan_and_notify_due_contracts,
    scan_and_notify_expiring_contracts,
)
from tests.conftest import (
    make_org,
    make_contract,
    make_contract_assignment,
...
def test_contract_due_soon_notifies_each_assigned_staff_user(db_session):
    org = make_org(db_session)
    lawyer, _ = make_staff(db_session, org)
    paralegal, _ = make_staff(db_session, org, email="paralegal@example.com")
    contract = make_contract(db_session, org, due_date=TODAY + timedelta(days=3))
    make_contract_assignment(db_session, contract, lawyer)
    make_contract_assignment(db_session, contract, paralegal)

    created = scan_and_notify_due_contracts(db_session)
    ...
    assert all(n.target_id == contract.id for n in notifications)


def test_contract_not_due_soon_excluded(db_session):
    org = make_org(db_session)
    lawyer, _ = make_staff(db_session, org)
    contract = make_contract(db_session, org, due_date=TODAY + timedelta(days=30))
    make_contract_assignment(db_session, contract, lawyer)

    assert scan_and_notify_due_contracts(db_session) == 0


def test_contract_no_due_date_excluded(db_session):
    org = make_org(db_session)
    lawyer, _ = make_staff(db_session, org)
...
def test_contract_closed_or_declined_excluded(db_session):
    org = make_org(db_session)
    lawyer, _ = make_staff(db_session, org)
    for status in (ContractStage.CLOSED, ContractStage.DECLINED):
        contract = make_contract(db_session, org, due_date=TODAY + timedelta(days=1), status=status)
        make_contract_assignment(db_session, contract, lawyer)

    assert scan_and_notify_due_contracts(db_session) == 0


def test_contract_inactive_assignee_excluded(db_session):
    org = make_org(db_session)
    lawyer, _ = make_staff(db_session, org, is_active=False)
    contract = make_contract(db_session, org, due_date=TODAY + timedelta(days=1))
    make_contract_assignment(db_session, contract, lawyer)
...
def test_contract_due_soon_scan_twice_same_window_does_not_duplicate(db_session):
    org = make_org(db_session)
    lawyer, _ = make_staff(db_session, org)
    contract = make_contract(db_session, org, due_date=TODAY + timedelta(days=3))
    make_contract_assignment(db_session, contract, lawyer)

    first_run = scan_and_notify_due_contracts(db_session)
    second_run = scan_and_notify_due_contracts(db_session)
...
def test_contract_rescheduled_due_date_within_window_does_not_resend_same_day(db_session):
    """Dedup is anchored on "already reminded about this contract recently", not on the due
    ...
    assert scan_and_notify_due_contracts(db_session) == 1

    contract.due_date = TODAY + timedelta(days=6)
    db_session.flush()
