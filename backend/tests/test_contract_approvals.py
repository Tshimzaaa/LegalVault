"""
Contract status transition rules + the approval gate on awaiting_signature -> signed and
signed -> closed. See CONTRACT_STATUS_TRANSITIONS / GATED_TRANSITIONS in
app/modules/contracts/service.py for the graph these tests exercise.
"""
from app.modules.auth.models.role import UserRole
from tests.conftest import TwoOrgs, auth_headers, make_org, make_contract, make_staff


def _advance_to_awaiting_signature(client, contract_id, admin):
    client.patch(f"/contracts/{contract_id}/status", json={"status": "in_review"}, headers=auth_headers(admin))
    client.patch(
        f"/contracts/{contract_id}/status", json={"status": "awaiting_signature"}, headers=auth_headers(admin)
    )
...
def test_invalid_transition_rejected(client, db_session):
    ...
    contract = make_contract(db_session, org)  # status: intake

    res = client.patch(f"/contracts/{contract.id}/status", json={"status": "signed"}, headers=auth_headers(admin))
...
def test_same_status_is_a_noop(client, db_session):
    ...
    res = client.patch(f"/contracts/{contract.id}/status", json={"status": "intake"}, headers=auth_headers(admin))
...
def test_gated_transition_rejected_via_direct_patch(client, db_session):
    ...
    admin, _ = make_staff(db_session, org)
    contract = make_contract(db_session, org)
    _advance_to_awaiting_signature(client, contract.id, admin)
...
def test_request_approval_creates_pending_and_notifies_eligible_approvers(client, db_session):
    ...
    res = client.post(
        f"/contracts/{contract.id}/approvals", json={"to_status": "signed"}, headers=auth_headers(paralegal)
    )
    ...
    # contract status hasn't moved yet — still pending
    get_res = client.get(f"/contracts/{contract.id}", headers=auth_headers(admin))
    assert get_res.json()["status"] == "awaiting_signature"

    admin_notifs = client.get("/notifications", headers=auth_headers(admin)).json()
    assert any(n["type"] == "contract.approval_requested" for n in admin_notifs)

    lawyer_notifs = client.get("/notifications", headers=auth_headers(lawyer)).json()
    assert any(n["type"] == "contract.approval_requested" for n in lawyer_notifs)

    # requester doesn't notify themselves
    paralegal_notifs = client.get("/notifications", headers=auth_headers(paralegal)).json()
    assert not any(n["type"] == "contract.approval_requested" for n in paralegal_notifs)
...
def test_duplicate_pending_request_rejected(client, db_session):
    ...
    first = client.post(
        f"/contracts/{contract.id}/approvals", json={"to_status": "signed"}, headers=auth_headers(admin)
    )
...
def test_decide_approve_applies_status_and_notifies_requester(client, db_session):
    ...
    decide_res = client.patch(
        f"/contracts/{contract.id}/approvals/{approval['id']}",
    ...
    paralegal_notifs = client.get("/notifications", headers=auth_headers(paralegal)).json()
    assert any(n["type"] == "contract.approval_decided" for n in paralegal_notifs)
...
def test_non_eligible_actor_cannot_decide(client, db_session):
    ...
    approval = client.post(
        f"/contracts/{contract.id}/approvals", json={"to_status": "signed"}, headers=auth_headers(requester)
    ).json()

    # other_paralegal passes the route-level role check (paralegal is case-work) but isn't
    # admin and isn't this contract's lead lawyer — the service-level eligibility check.
    decide_res = client.patch(
        f"/contracts/{contract.id}/approvals/{approval['id']}",
        json={"decision": "approved"},
...
def test_pending_approvals_scoped_to_org(client, db_session):
    ...
    _advance_to_awaiting_signature(client, tf.contract_a.id, tf.staff_a)
    client.post(
        f"/contracts/{tf.contract_a.id}/approvals", json={"to_status": "signed"}, headers=auth_headers(tf.staff_a)
    )

    org_a_pending = client.get("/contracts/approvals/pending", headers=auth_headers(tf.staff_a)).json()
    assert len(org_a_pending) == 1
    assert org_a_pending[0]["contract_id"] == str(tf.contract_a.id)

    org_b_pending = client.get("/contracts/approvals/pending", headers=auth_headers(tf.staff_b)).json()
